def generate_conditions(table_data):
    # Split the table data by lines and commas
    rows = [row.split(",") for row in table_data.strip().split("\n")]
    
    # Extract the headers and ignore them
    headers = rows[0]
...
formatted_conditions = generate_conditions(table_data)
print(formatted_conditions)
